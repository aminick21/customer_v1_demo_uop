...
class _ChatScreenState extends State<ChatScreen> {
  ...
  _sendMessage(String content, Url url, String videoThumbnail) async {
    ...
    if (url.mime.contains('image')) {
      message.content =
          '{} sent an image'.tr(args: ['${MyAppState.currentUser!.firstName}']);
    } else if (url.mime.contains('video')) {
      message.content =
          '{} sent a video'.tr(args: ['${MyAppState.currentUser!.firstName}']);
    ...
    if (await _checkChannelNullability(
      ...
      await _fireStoreUtils.sendMessage(homeConversationModel.members, message,
          homeConversationModel.conversationModel!);
      ...
      showAlertDialog(context, 'An error occurred'.tr(),
          'Failed to send message'.tr(), true);
    }
  }
          ...
          onPressed: () async {
            ...
            if (isSuccessful) {
              ...
              _showAlertDialog(
                  context,
                  'Report'.tr(),
                  '{} has been blocked and reported.'.tr(args: [
                    '${homeConversationModel.members.first.fullName()}'
                  ]));
            } else {
              _showAlertDialog(
                  context,
                  'Report'.tr(),
                  'Couldn\'t report or block {}'.tr(args: [
                    '${homeConversationModel.members.first.fullName()}'
                  ]));
            }
          },
